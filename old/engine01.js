...
function renderCategoryButtons() {
    ...
    if (adminFilters) adminHtml = adminHtml;
...
function renderMatrix() {
    ...
    activeCategories.forEach((cat, idx) => {
        ...
        hHtml += `<th title="${cData.desc}" class="py-3 px-1 text-center ${colorSet[idx % colorSet.length]} border-l border-stone-700/50 w-[10%] bg-[#292524] shadow-sm cursor-help">${cData.name}</th>`;
    });
...
function executeMatrixAuction() {
    ...
    showModal("🚨 DEAL PERFEKT!", `<strong>${activeMatrixAuctionItem.name}</strong> geht für <strong>${price.toLocaleString()} ${Config.currency.symbol}</strong> an <strong>${m.name}</strong>!${sapMsg}${cbText}`, () => {
        setTimeout(() => { openJokerPhase(); }, 3000);
    });
...
function useSkipJoker(id) {
    ...
    showConfirmModal(`Item <strong>${activeMatrixAuctionItem.name}</strong> ins Klo werfen?`, () => {
        ...
        if (list && list.length > 0) {
            ...
            if ($('matrix-active-bid-desc')) $('matrix-active-bid-desc').innerHTML = newItem.desc + getTagsHtml(newItem);
...
function renderJokerPhaseModal() {
    ...
    playerJokerOrder.forEach(mgrId => {
        ...
        if (m.id === blockedPlayerId) {
            container.innerHTML += `
                <div class="flex flex-col md:flex-row items-center justify-between bg-red-950/20 p-2.5 rounded-lg border border-red-900/40 mb-2 gap-2 opacity-60">
                    <span class="font-black text-red-400 w-full md:w-1/4 text-left">${m.name}</span>
                    <div class="text-xs font-bold text-red-400 w-full md:w-3/4 justify-end flex items-center">
                        <i class="fa-solid fa-handcuffs mr-1"></i> In Isolationshaft (Keine Aktionen möglich)
                    </div>
                </div>`;
            return;
        }
        ...
        if (m.jokers.block > 0) {
            if (jokerPhaseState.blockUsed) {
                btnHtml += `<button title="Ein anderer Spieler sitzt bereits in Isolationshaft" class="bg-stone-900 text-stone-500 line-through opacity-50 cursor-not-allowed text-[10px] font-bold px-2 py-1.5 rounded border border-stone-800">🛑 Block (${m.jokers.block})</button>`;
            } else {
                btnHtml += `<button onclick="triggerPhaseJoker(${m.id}, 'block')" class="bg-red-900/40 text-[10px] font-bold text-white px-2 py-1.5 rounded border border-red-500 hover:bg-red-600 transition">🛑 Block (${m.jokers.block})</button>`;
            }
        }

        if (m.jokers.autoBuy > 0) {
            let abDis = jokerPhaseState.autoBuyUsed ? 'opacity-50 cursor-not-allowed grayscale' : 'hover:bg-purple-600 border-purple-500';
            let abClick = jokerPhaseState.autoBuyUsed ? '' : `onclick="triggerPhaseJoker(${m.id}, 'autoBuy')"`;
            btnHtml += `<button ${abClick} class="bg-purple-900/40 text-[10px] font-bold text-white px-2 py-1.5 rounded border transition ${abDis}">🎯 Bestechen (${m.jokers.autoBuy})</button>`;
        }

        if (m.jokers.bonus > 0) {
            btnHtml += `<button onclick="triggerPhaseJoker(${m.id}, 'bonus')" class="bg-green-900/40 text-[10px] font-bold text-white px-2 py-1.5 rounded border border-green-500 hover:bg-green-600 transition">💰 Schutzgeld (${m.jokers.bonus})</button>`;
        }

        if (m.jokers.gamble > 0) {
            btnHtml += `<button onclick="triggerPhaseJoker(${m.id}, 'gamble')" class="bg-orange-900/40 text-[10px] font-bold text-white px-2 py-1.5 rounded border border-orange-500 hover:bg-orange-600 transition">🎓 Schmuggel (${m.jokers.gamble})</button>`;
        }

        if (btnHtml === '') {
            btnHtml = `<span class="text-[10px] text-stone-500 italic">Keine Joker übrig</span>`;
        }

        container.innerHTML += `
            <div class="flex flex-col md:flex-row items-center justify-between bg-black/40 p-2.5 rounded-lg border border-stone-700 mb-2 gap-2">
                <span class="font-black text-orange-400 w-full md:w-1/4 text-left">${m.name}</span>
                <div class="flex flex-wrap gap-1.5 w-full md:w-3/4 justify-start md:justify-end">
                    ${btnHtml}
                </div>
            </div>`;
    });
