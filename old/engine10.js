...
for (let jKey in Config.terminology.jokers) {
    if (m.jokers[jKey] > 0) { 
        ...
        let cls = `relative w-6 h-6 flex justify-center items-center rounded bg-transparent text-stone-500 hover:text-${c}-400 hover:bg-[#292524] transition-all`;
...
jokersHtml += `</div>`;  
        ...
        let perkBadge = m.perk !== 'NONE' ? `<div class="text-[9px] font-bold text-indigo-400 bg-indigo-950/30 px-1 py-0.5 " title="${Config.perks[m.perk]?.desc || ''}">${Config.perks[m.perk]?.name || m.perk}</div>` : '';
        ...
        let syn = 0; for (let t in tc) if (tc[t] > 1) syn += getConf('scoring','synergyPointsPerTag');
...
tbody.innerHTML += `
    ...
    <div class="flex flex-col gap-1 w-full">
        ...
        <div class="flex items-center justify-between gap-2 pb-0.5">
            ...
            <span class="text-xs font-black text-amber-400 bg-amber-950/40 px-2 py-0.5 rounded whitespace-nowrap cursor-help shadow-sm" title="Basis: ${bp} Pkt. | Synergie: +${syn} Pkt.">
        ...
        <div class="relative mt-1.5 w-full bg-black/20 px-1.5 py-1 rounded border border-stone-700/30 flex items-center gap-2">
            <span id="budget-display-${m.id}" class="text-[10px] font-black ${textColor} transition-colors whitespace-nowrap">
                ${m.budget.toLocaleString()} ${Config.currency.symbol}
            </span>
            <div class="flex-1 bg-stone-800 rounded-full h-1.5 overflow-hidden border border-stone-700">
...
function processPurchase(manager, price, item, cat) {
    ...
    if (buyerHadCashback) {
        cashbackVal = Math.floor(getConf('mechanics','bonusCashbackFraction'));
...
function playAuctionAnimation(price) {
    ...
    setTimeout(() => { h.classList.add('hidden'); p.classList.add('hidden'); }, getConf('ui','auctionHammerDuration'));
}
...
function spinBonusWheelAll() {
    ...
    if (!highRollerTriggered && Math.random() < getConf('roulette','highRollerChance')) {
...
function triggerEndgame() {
    ...
    if (loser.totalPoints > 0) { let bonus = Math.floor(loser.totalPoints * getConf('scoring','loserBonusFraction')); loser.totalPoints += bonus; loser.loserBonusText = loser.loserBonusText ? loser.loserBonusText + `, +${bonus} (Mitleid)` : `+${bonus} (Mitleid)`; }
