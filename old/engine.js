...
let managers = []; let activeCategories = []; let countManagers = 4; let startingBudget = 1000000;
...
function applyConfigToUI() {
  ...
  if ($('ui-game-title')) {
    ...
    // Joker-Legende in Matrix-View befüllen
    ...
    if (jLegend && Config.terminology.jokers) {
        ...
        for (let key in Config.terminology.jokers) {
            ...
            html += `<div class="flex items-center gap-2 w-full md:w-auto"><span class="text-${c}-400 font-bold bg-black px-2 py-1 rounded border border-${c}-900/40">${j.icon} ${j.label}</span><span>${descMap[key] || ''}</span></div>`;
        }
...
function renderManagerCountButtons() {
    const container = $('mgr-count-buttons'); if (!container) return;
    container.innerHTML = '';
    for (let i = 2; i <= 7; i++) {
        const isActive = (i === countManagers);
        const cls = isActive ? `bg-orange-500 text-white` : `bg-[#292524] text-stone-400 hover:bg-stone-700 hover:text-white`;
        container.innerHTML += `<button onclick="promptRestart(${i})" id="btn-mgr-${i}" class="px-3 py-1 rounded text-xs font-bold ${cls} transition-all-custom">${i}</button>`;
    }
}
...
function importDB(e) {
    ...
    reader.onload = function (event) {
        ...
        } catch (err) { showModal("Kritischer Fehler", "Formatfehler in der JSON."); }
        e.target.value = '';
    };
...
function renderCategoryButtons() {
    const catSelector = $('matrix-category-selector'); const adminFilters = $('admin-category-filters');
    let matrixHtml = ''; let adminHtml = '';
    for (let catKey in Config.categories) {
        let catData = Config.categories[catKey];
        if (activeCategories.includes(catKey)) {
            matrixHtml += `<button onclick="selectMatrixAuctionCategory('${catKey}', this)" class="matrix-cat-btn flex-1 bg-[#292524] hover:bg-stone-700 text-stone-300 text-[10px] font-bold py-2 rounded border border-stone-600 transition truncate">${catData.icon} ${catData.name}</button>`;
        }
    ...
    if (catSelector) catSelector.innerHTML = matrixHtml;
    if (adminFilters) adminFilters.innerHTML = adminHtml;
    // Activate first admin category visually
...
function renderMatrix() {
    ...
    const colorSet = ['red-400', 'orange-400', 'purple-400', 'rose-400', 'yellow-400', 'green-400', 'cyan-400'];
    activeCategories.forEach((cat, idx) => {
        let cData = Config.categories[cat];
        hHtml += `<th title="${cData.desc}" class="py-3 px-1 text-center text-${colorSet[idx % colorSet.length]} border-l border-stone-700/50 w-[10%] bg-[#292524] shadow-sm cursor-help">${cData.name}</th>`;
    });
    hHtml += `<th class="py-3 px-1 text-center text-lime-400 border-l border-stone-700/50 w-[5%] font-black tracking-widest bg-[#292524] shadow-sm"><i class="fa-solid fa-leaf"></i> Pkt.</th><th class="py-3 px-3 text-right border-l border-stone-700 text-orange-400 w-[10%] bg-[#292524] shadow-sm">${Config.currency.name} (${Config.currency.symbol})</th>`;
    thead.innerHTML = hHtml;
    tbody.innerHTML = '';

    managers.forEach(m => {
        const isBlk = m.id === blockedPlayerId;
        let jokersHtml = `<div class="flex flex-wrap w-full gap-0.5 mt-1.5">`;
        ...
        for (let jKey in Config.terminology.jokers) {
            ...
            let hasJoker = m.jokers[jKey] > 0; let countLabel = m.jokers[jKey] > 1 ? ` (${m.jokers[jKey]})` : '';
            let c = jData.color || 'stone';
            let cls = hasJoker ? `bg-${c}-500/20 text-${c}-400 border border-${c}-500/30 hover:bg-${c}-500 hover:text-black` : 'bg-black text-stone-600 border border-stone-800 line-through cursor-not-allowed';
            jokersHtml += `<button onclick="${fnMap[jKey]}(${m.id})" title="${jData.label}" class="flex-1 flex justify-center items-center text-[7px] font-black px-0 py-0.5 rounded transition whitespace-nowrap ${cls}">${jData.icon} ${jData.label}${countLabel}</button>`;
        }
        jokersHtml += `</div>`;
        ...
        tbody.innerHTML += `
            <tr class="${isBlk ? 'bg-red-950/40 border-l-4 border-red-500 grayscale transition opacity-80' : 'hover:bg-[#1c1917]/30 transition'}">
                <td class="py-4 px-3 border-r border-stone-700/80"><div class="flex flex-col gap-1">${blkOverlay}<input type="text" value="${m.name}" onchange="renameManager(${m.id}, this.value)" class="bg-transparent text-sm font-black text-white border-b border-transparent hover:border-stone-600 focus:border-orange-400 focus:outline-none w-full">${perkBadge}${cbBadge}${jokersHtml}</div></td>
                ${slots}
                <td class="py-4 px-1 text-center border-l border-stone-700/80 bg-blue-950/20"><div class="text-xl font-black text-lime-400">${bp + syn}</div><div class="text-[8px] text-stone-400 uppercase tracking-widest mt-1">Basis: ${bp}</div><div class="text-[8px] text-yellow-400 font-bold uppercase tracking-widest">+${syn} Syn</div></td>
                <td class="relative py-4 px-2 text-right font-black ${textColor} text-sm border-l border-stone-700/80 align-top">
                    <span id="budget-display-${m.id}" class="transition-colors block relative mb-1">${m.budget.toLocaleString()} ${Config.currency.symbol}</span>
                    <div class="w-full bg-stone-800 rounded-full h-1.5 mt-1 overflow-hidden border border-stone-700">
...
function renderSlotCell(mId, k, pObj) {
    ...
    if (!pObj) return `<td class="py-2 px-1 text-center border-l border-stone-700/40"><div class="border border-dashed border-stone-600/60 rounded-lg p-2 h-[45px] flex items-center justify-center text-stone-600 text-xs font-semibold ${specialBorderClass}">- Leer -</div></td>`;
    ...
    let scoreHtml = isBoosted ? `<i class="fa-solid fa-arrow-trend-up text-[6px] text-yellow-400"></i> <span class="text-yellow-400 font-black animate-pulse">${displayScore}</span>` : `<i class="fa-solid fa-leaf text-[6px] text-lime-400"></i> ${displayScore}`;
    return `<td class="py-2 px-0.5 text-center border-l border-stone-700/40 relative group/cell"><div onclick="removePlayerFromMatrix(${mId}, '${k}')" class="group/card relative cursor-pointer bg-black/60 hover:bg-red-950/30 hover:border-red-900 rounded-lg p-1 transition flex flex-col items-center justify-center h-[65px] border-b-[4px] ${bb} ${specialBorderClass}"><div class="absolute top-0 right-0 bg-orange-900/80 text-white text-[8px] font-bold px-1 py-0.5 rounded-bl shadow z-10">${scoreHtml}</div><span class="text-2xl mb-1 group-hover/card:hidden">${pObj.icon}</span><span class="text-2xl mb-1 hidden group-hover/card:inline text-red-500"><i class="fa-solid fa-toilet"></i></span><span class="text-[9px] font-bold text-white leading-tight w-full px-0.5 line-clamp-2 group-hover/card:text-red-400">${pObj.name}</span></div><div class="absolute top-[90px] left-1/2 transform -translate-x-1/2 w-56 bg-stone-800 border-2 border-stone-600 text-stone-200 p-3 rounded-xl shadow-2xl opacity-0 invisible group-hover/cell:opacity-100 group-hover/cell:visible transition-all duration-200 z-[100] pointer-events-none flex flex-col items-start text-left"><div class="font-black text-white text-sm mb-1 leading-tight">${pObj.name}</div><div class="text-[11px] italic leading-snug text-stone-400 mb-2">${pObj.desc}</div><div class="mt-auto text-lime-400 font-bold text-xs w-full text-right">${pObj.cost.toLocaleString()} ${Config.currency.symbol}</div></div></td>`;
}
...
// =====================================================================
// POOL MANAGEMENT (returnToPool, returnAllToPool)
// =====================================================================
...
function showPerksModal() {
    ...
    managers.forEach(m => {
        ...
        phtml += `<div class="mb-4 bg-stone-800 p-3 rounded-lg border border-stone-600"><label class="block text-sm font-black text-orange-400 mb-1.5">${m.name}</label><select id="perk-select-${m.id}" onchange="updatePerkDesc(${m.id})" class="w-full bg-[#1c1917] border border-stone-500 text-white rounded p-2 text-xs outline-none font-bold shadow-inner">${opts}</select><p class="text-[10px] text-stone-300 mt-2 italic leading-snug" id="perk-desc-${m.id}">${Config.perks[assigned].desc}</p></div>`;
    });
...
function startGameWithoutPerks() {
    managers.forEach(m => { m.perk = 'NONE'; m.protegeCats = []; m.jokers = { block: 1, bonus: 1, autoBuy: 1, gamble: 1, skip: 1 }; });
    if ($('perks-modal')) $('perks-modal').classList.add('hidden');
    renderMatrix(); updateBuyerDropdown(); renderAdminPool(); renderEventsPool(); updateEventChanceDisplays();
...
function selectMatrixAuctionCategory(cat, btn) {
    ...
    document.querySelectorAll('.matrix-cat-btn').forEach(b => b.className = "matrix-cat-btn flex-1 bg-[#292524] hover:bg-stone-700 text-stone-300 text-[10px] font-bold py-2 rounded border border-stone-600 transition truncate");
    if (!btn) document.querySelectorAll('.matrix-cat-btn').forEach(b => { if (b.getAttribute('onclick')?.includes(`'${cat}'`)) btn = b; });
    if (btn) btn.className = "matrix-cat-btn flex-1 bg-stone-200 text-black text-[10px] font-black py-2 rounded border-2 border-white shadow-xl transition truncate scale-105";
...
function drawMatrixRandomPlayer() {
    ...
    if (!list || list.length === 0) return showModal("Katalog leer!", "Kategorie komplett leergeräumt.");
    ...
    if (blindDrawsLeft > 0) {
        ...
        $('matrix-active-bid-desc').innerHTML = `<span class="text-red-400 font-bold">Unleserlich. Blindflug!</span>`;
    } else {
        $('matrix-active-bid-target').innerHTML = `<span class="text-xl">${item.icon}</span> ${item.name}`;
        $('matrix-active-bid-start-price').innerText = `Kosten: ${item.cost.toLocaleString()} ${Config.currency.symbol}`;
        $('matrix-active-bid-desc').innerHTML = item.desc + getTagsHtml(item);
    }
...
function selectAuctionItem(id) {
    ...
    if (item) {
        ...
        if ($('active-bid-desc')) $('active-bid-desc').innerHTML = (item.adminDesc || item.desc) + getTagsHtml(item);
...
function processPurchase(manager, price, item, cat) {
    ...
    if (buyerHadCashback) {
        ...
        if (manager.perk === 'perk4') cashbackVal = Math.floor(cashbackVal * 1.25);
...
function useAutoBuyJoker(id) {
    ...
    showConfirmModal(`Item erzwingen für 1,5x Preis (${Config.categories[activeMatrixAuctionCategory]?.name})?`, () => {
        const item = list[Math.floor(Math.random() * list.length)]; const price = Math.floor(item.cost * 1.5);
...
function useSkipJoker(id) {
    ...
    showConfirmModal(`Item <strong>${activeMatrixAuctionItem.name}</strong> ins Klo werfen?`, () => {
        ...
        resetMatrixActiveBid(); addLog(`Gespült: ${m.name} vernichtet Objekt.`, "alert");
        showModal("🚽 WEGGESPÜLT", "Das Objekt wurde vernichtet!");
    });
}
...
// =====================================================================
// SPECIAL AUKTIONEN (Blind / Dutch / Wheel)
// =====================================================================
...
function openWheelModal() {
    finishedSpins = 0; if ($('wheel-close-btn')) $('wheel-close-btn').classList.add('hidden');
    const c = $('wheels-grid'); if (!c) return; c.innerHTML = '';
    managers.forEach(m => {
        c.innerHTML += `<div class="bg-stone-900 border-2 border-stone-700 p-4 rounded-xl flex flex-col items-center w-44 shadow-2xl"><div class="text-xs font-black text-orange-400 mb-3 tracking-wide uppercase truncate w-full text-center">${m.name}</div><div class="relative w-full bg-black border border-stone-800 rounded-lg overflow-hidden flex justify-center shadow-inner" style="height: 96px;"><div class="absolute inset-x-0 border-y border-red-500/40 bg-red-500/5 pointer-events-none z-10" style="height: 32px; top: 32px;"></div><div id="reel-track-${m.id}" class="absolute flex flex-col items-center justify-start w-full" style="transform: translateY(0px); top: 32px;"><div class="h-8 flex items-center justify-center text-stone-500 text-xs italic" style="height:32px;">Bereit...</div></div></div><button id="spin-btn-${m.id}" onclick="spinIndividualReel(${m.id})" class="mt-4 w-full bg-gradient-to-r from-orange-600 to-yellow-500 text-stone-900 text-[11px] font-black py-2 rounded-lg transition shadow-md uppercase tracking-wider"><i class="fa-solid fa-play mr-1"></i> Start</button></div>`;
    });
    if ($('wheel-modal')) $('wheel-modal').classList.remove('hidden');
}

function spinIndividualReel(mgrId) {
    const btn = $(`spin-btn-${mgrId}`); if (!btn || btn.disabled) return; btn.disabled = true; btn.classList.add('opacity-50', 'cursor-not-allowed'); btn.innerText = "Läuft...";
    let vals = [10000, 20000, 30000, 40000, 50000, 60000, 70000, 80000, 90000, 100000, -50000];
    if (isHighRollerSession) vals = [20000, 40000, 60000, 80000, 100000, 250000, 250000];
    const t = $(`reel-track-${mgrId}`), win = vals[Math.floor(Math.random() * vals.length)];
    let html = ''; for (let i = 0; i < 35; i++) { let rv = vals[Math.floor(Math.random() * vals.length)], n = rv < 0; html += `<div class="h-8 flex items-center justify-center font-bold text-xs ${n ? 'text-red-400/60' : (rv === 250000 ? 'text-yellow-400/80' : 'text-stone-400/60')}" style="height:32px; line-height:32px;">${n ? '' : '+'}${rv.toLocaleString()}</div>`; }
    let wn = win < 0, jpot = win === 250000; let winColor = wn ? 'text-red-500 bg-red-950/40' : (jpot ? 'text-yellow-400 bg-yellow-900/40' : 'text-green-400 bg-green-950/40');
    html += `<div class="h-8 flex items-center justify-center font-black ${winColor}" style="height:32px; line-height:32px; font-size:13px;">${wn ? '' : '+'}${win.toLocaleString()}</div>`;
    t.innerHTML = html; t.style.transition = 'none'; t.style.transform = 'translateY(0px)'; void t.offsetHeight;
    t.style.transition = 'transform 4.5s cubic-bezier(0.15, 0.85, 0.3, 1)'; t.style.transform = `translateY(-${35 * 32}px)`;
    setTimeout(() => {
        const m = managers.find(x => x.id === mgrId); let actualWin = applyBudgetChange(m, win, "wheel");
        btn.innerText = "✓ Fertig"; addLog(`Rad: ${m.name} bekommt ${actualWin >= 0 ? '+' : ''}${actualWin.toLocaleString()}`, actualWin >= 0 ? "buy" : "alert");
        finishedSpins++; if (finishedSpins >= managers.length && $('wheel-close-btn')) $('wheel-close-btn').classList.remove('hidden');
    }, 4600);
}

function closeWheelModal() { if ($('wheel-modal')) $('wheel-modal').classList.add('hidden'); renderMatrix(); updateBuyerDropdown(); }
...
function selectManualMatrixCard(id, cat) {
    ...
    if ($('matrix-active-bid-desc')) $('matrix-active-bid-desc').innerHTML = item.desc + getTagsHtml(item);
    if ($('matrix-auction-price')) $('matrix-auction-price').value = item.cost;
...
function checkCategoryCompletion(cat) { if (completedCategoryNames.includes(cat)) return; if (managers.every(m => m.team[cat] !== null)) { completedCategoryNames.push(cat); if (specialEventsRemaining.length > 0) pendingSpecialEvent = true; } }
...
document.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'Escape') {
        const modals = ['shortcuts-modal', 'custom-modal', 'confirm-modal', 'edit-modal', 'category-modal', 'event-modal', 'block-modal', 'gamble-modal', 'blind-modal', 'dutch-modal', 'wheel-modal', 'perks-modal', 'gamble-choice-modal', 'perk-catalog-modal'];
        modals.forEach(id => {
            ...
            if (el && !el.classList.contains('hidden')) {
                ...
                else if (id === 'wheel-modal') { if ($('wheel-close-btn') && !$('wheel-close-btn').classList.contains('hidden')) closeWheelModal(); }
